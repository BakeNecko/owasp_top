...
async def startup_event():
    ...
    for data in USER_DATA:
        if not get_user(username=data.get('username')):
            create_user(UserCreateSerializer(**data), is_admin=False)
...
async def delete_user_by_username(username: str):
    try:
        ...
        return {'res': f'User Delete Success!'}
    except Exception as err:
        return {'res': f"Can't delete user: detail: {err}"}
